 ...
 *
 * Cards, panels, tables and tab rows use the DSH theme tokens
 * (--dsw-alias-* / --dsh-*), so every surface follows the active theme in
...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var turnCostRow = {
      ...
      fontSize: 11, lineHeight: '16px', color: T.label3,
    ...
    function TurnCost(props) {
      ...
      var items = [
        ...
        el('span', { key: 'v', style: { color: T.label2, fontWeight: 500 } }, fmtCost(row.usd, currency, fx)),
      ]
      if (row.calls > 1) {
        items.push(el('span', { key: 's1', style: { opacity: 0.5 } }, '·'))
      ...
      if (cacheHit !== null) {
        items.push(el('span', { key: 's2', style: { opacity: 0.5 } }, '·'))
    ...
    //
    // The sidebar collapses to a 56px rail, where `wide` goes false — then it
    ...
    function SidebarSpend(props) {
      ...
      return el('button', {
        ...
        style: {
          ...
          padding: wide ? '4px 10px' : '4px 0', justifyContent: wide ? 'flex-start' : 'center',
          color: color, fontSize: 12, lineHeight: '18px', minWidth: 0,
        },
      },
        wide ? el('span', { style: { color: T.label3, flexShrink: 0 } }, t('sidebar.today')) : null,
        wide
          ? el('span', { style: { fontVariantNumeric: 'tabular-nums', fontWeight: 500 } },
              fmtCost(todayUsd, budget.currency || 'CNY', fx))
          : null,
        pct !== null
          ? el('span', {
              style: {
                marginLeft: wide ? 'auto' : 0, fontSize: 11, flexShrink: 0,
                fontVariantNumeric: 'tabular-nums',
              },
            }, Math.round(pct) + '%')
          : null)
    ...
    function BudgetSection(props) {
      ...
      var editor = el('div', { style: { display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', marginTop: 8 } },
        el('span', { style: segStyle }, periods.map(function (p) {
          return el('button', {
            key: p,
            style: budget.period === p ? segBtnOn : segBtn,
        ...
        el('input', {
          ...
          style: {
            width: 92, border: '1px solid ' + T.border, background: 'transparent', color: T.label,
            borderRadius: 8, padding: '3px 8px', fontSize: 12, outline: 'none',
            fontVariantNumeric: 'tabular-nums',
          },
        }),
        el('span', { style: { fontSize: 11, color: T.label3 } }, budget.currency),
        budget.amount > 0
          ? el('button', {
              style: { ...segBtn, border: '1px solid ' + T.border, borderRadius: 8, color: T.label3 },
      ...
      if (!(budget.amount > 0) && !isEditing) {
        return el('div', { style: section },
          ...
          el('button', {
            style: { ...segBtn, border: '1px solid ' + T.border, borderRadius: 8, color: T.label },
      ...
      return el('div', { style: section },
        el('div', { style: { display: 'flex', alignItems: 'baseline', gap: 8, minWidth: 0 } },
          ...
          el('span', { style: { fontSize: 11, color: T.label3 } }, t('budget.period.' + budget.period)),
          el('span', {
            style: {
        ...
        el('div', { style: { height: 6, borderRadius: 3, background: T.hover, overflow: 'hidden', marginTop: 8 } },
          el('div', {
            style: {
        ...
        el('div', { style: { fontSize: 11, color: T.label3, marginTop: 4, fontVariantNumeric: 'tabular-nums' } },
    ...
    function Dashboard(props) {
      ...
      return el('div', {
        style: {
          padding: '16px 16px', width: '100%', maxWidth: 1080, margin: '0 auto',
                  ...
                  (d.bySession || []).length > 1
                    ? el('div', { style: section },
                        ...
                        (d.bySession || []).map(function (row) {
                          ...
                          return el('div', { key: row.sessionId, style: { marginTop: 8, minWidth: 0 } },
                            el('div', { style: { display: 'flex', alignItems: 'baseline', gap: 8, minWidth: 0 } },
                              el('span', {
                                style: {
                                  fontSize: 12, color: T.label, minWidth: 0, overflow: 'hidden',
                                  textOverflow: 'ellipsis', whiteSpace: 'nowrap',
                                },
                                title: row.sessionId,
                              }, sessionTitle(row.sessionId) || t('session.untitled')),
                              el('span', { style: { fontSize: 11, color: T.label3, flexShrink: 0 } },
                                row.calls + t('session.calls')),
                              el('span', {
                                style: {
                                  marginLeft: 'auto', fontSize: 12, fontWeight: 600, color: T.label,
                                  fontVariantNumeric: 'tabular-nums', flexShrink: 0,
                                },
                              }, fmtCost(row.usd, currency, fx))),
                            el('div', { style: { height: 3, borderRadius: 2, background: T.hover, overflow: 'hidden', marginTop: 3 } },
    ...
    function BillView(props) {
      return el('div', { style: { height: '100%', overflowY: 'auto', minWidth: 0 } },
        el(Dashboard, props))
    }
    ...
    function BillSettings(props) {
      ...
      return el('div', { style: { padding: '16px 16px', width: '100%', maxWidth: 720, boxSizing: 'border-box', minWidth: 0 } },
        el('div', { style: { fontSize: 15, fontWeight: 600, color: T.label } }, t('section.title')),
        el('div', { style: { fontSize: 11, color: T.label3, marginTop: 2, marginBottom: 14 } },
          t('settings.desc')),
        ...
        budgetScope && d && d.periods
          ...
          : el('div', { style: { fontSize: 12, color: T.label3 } },
