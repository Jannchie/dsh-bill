...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = `
      ...
      /* The option list is a separate popup document, not part of this page:
         Chromium copies the select's computed colour and background into it
         verbatim and paints its own body white underneath. This select is
         transparent on purpose (the pill takes its surface from the page), so
         in the dark palette the popup ended up as near-white text on that
         white body — an apparently empty list. Options get an opaque surface
         of their own, from the same tokens, so the popup follows the palette
         the way the closed control already does. */
      .dshbill-select option,.dshbill-select optgroup{color:${T.label};
        background:${T.layer2}}
      .dshbill-select optgroup{color:${T.label2}}
    ...
    var DICT_ZH = {
      ...
      'display.title': '显示',
      'display.desc': '数字怎么显示。这是全局设置,不是每个位置各一份。',
      'display.currency': '货币',
      'display.currency.desc': '作用于常驻费用行、每轮成本、侧边栏与「费用」标签页。预算保留自己设定的货币。',
      'currency.common': '常用',
      'currency.other': '其他',
      'surfaces.title': '显示位置',
      ...
      'tools.title': '工具',
      'tools.desc': '控制 bill_stats 工具对模型的可见性。回看窗口为 0 时工具完全不提供;设为 N 时,只有最近 N 条用户消息匹配下面的正则才会把工具的参数说明发给模型。',
      'tools.lazy': 'bill_stats 回看窗口',
      'tools.lazy.desc': '0 = 不提供工具;N = 扫描最近 N 条用户消息,匹配正则才提供。留空按 0 处理。',
      'tools.pattern': '匹配正则',
      'tools.pattern.desc': '对最近几条用户消息做不区分大小写的匹配;留空表示始终提供(回看窗口 > 0 时)。',
      'tools.pattern.invalid': '正则无法编译,未保存',
      'tools.placeholder': '0 = 不提供',
      'turn.cost': '本轮',
      'turn.steps': ' 步',
      'turn.cache': '缓存命中 ',
      'turn.tipIn': '新增输入 ',
    ...
    var DICT_EN = {
      ...
      'display.desc': 'How figures are shown. One setting, not one per surface.',
      'display.currency': 'Currency',
      'display.currency.desc': 'Used by the cost line, per-turn cost, sidebar and the Cost tab. The budget keeps its own.',
      'currency.common': 'Common',
      ...
      'tools.title': 'Tools',
      'tools.desc': 'Controls how visible the bill_stats tool is to the model. A lookback window of 0 hides the tool entirely; with N, the tool schema is only sent when the recent N user messages match the pattern below; other requests skip that overhead.',
      'tools.lazy': 'bill_stats lookback window',
      'tools.lazy.desc': '0 = never provide the tool; N = scan the recent N user messages and provide only when the pattern matches. Empty is treated as 0.',
      'tools.pattern': 'Match pattern',
      'tools.pattern.desc': 'Case-insensitive match against the recent user messages; leave empty to always provide (when the window is > 0).',
      'tools.pattern.invalid': 'The pattern does not compile; not saved',
      'tools.placeholder': '0 = never provide',
      'turn.cost': 'This turn',
      'turn.steps': ' steps',
    ...
    var PREFS_DEFAULT = {
      ...
      // The host owns the built-in pattern; this placeholder only covers the
      // instant before the first `prefs` reply lands, and is replaced by the
      // host's normalized value on that reply. The shipped default window is
      // 6 (provide on demand over the recent 6 user messages); 0 = never.
      lazyToolsLookback: 6, lazyToolsPattern: '',
    }
    ...
    /**
     * The bill_stats lookback window preference.
     *
     * Read through the same preference store as every other switch here, so
     * the settings row, the host-side judgement, and a second window all see
     * the same value. The shipped default is 6 (provide on demand over the
     * recent 6 user messages); 0 means "never provide the tool"; a positive
     * N scans the recent N user messages.
     */
    function useLazyTools() {
      var value = usePrefs(function (p) {
        return { lookback: Number(p.lazyToolsLookback) || 0, pattern: typeof p.lazyToolsPattern === 'string' ? p.lazyToolsPattern : '' }
      })
      var write = function (patch) {
        var stored = {}
        if (patch.lookback !== undefined) stored.lazyToolsLookback = patch.lookback
        if (patch.pattern !== undefined) stored.lazyToolsPattern = String(patch.pattern)
        prefsWrite(stored)
      }
      return [value, write]
    }
    ...
    function CostLine(props) {
      ...
      // Hook call sits above the early returns below, so the hook order is the
      // same whether or not this line ends up drawing anything.
    ...
    /**
     * The live fx table runs to ~170 currencies. The dozen in `CURRENCIES`
     * are pinned first, and the two halves sit in their own optgroups so the
     * long alphabetical tail reads as a separate, scannable section rather
     * than a wall the pinned few blend into. A native select already jumps
     * on type-ahead, so no search field is needed for the tail.
     */
    function CurrencySelect(props) {
      ...
      return el('select', {
        ...
        el('optgroup', { key: 'common', label: t('currency.common') }, common.map(option)),
        other.length > 0
          ? el('optgroup', { key: 'other', label: t('currency.other') }, other.map(option))
          : null)
    }
      ...
      return el('div', {
        ...
        el('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 10, flexWrap: 'wrap', marginBottom: 14, minWidth: 0 } },
          ...
          el('div', { style: { display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' } },
            ...
            el(CurrencySelect, { t: t, value: currency, fx: fx, onChange: setCurrency }))),
     ...
     * Display currency.
     *
     * One row for what used to be a per-surface accident: the dock line and
     * the per-turn line had no picker at all, and the report's picker was
     * local state that reset on reload. Laid out like the surfaces rows below
     * it — label and hint left, control right — because a select is what the
     * shipped General page uses for a one-of-N preference.
     */
    function DisplaySection(props) {
      var t = props.t
      var pair = useDisplayCurrency()
      return el('div', { style: section },
        el('div', { style: panelT }, t('display.title')),
        el('div', { style: panelSub }, t('display.desc')),
        el('div', { style: surfaceRow },
          el('div', { style: { minWidth: 0 } },
            el('div', { style: surfaceName }, t('display.currency')),
            el('div', { style: surfaceHint }, t('display.currency.desc'))),
          el(CurrencySelect, { t: t, value: pair[0], fx: props.fx, onChange: pair[1] })))
    }

    /**
     * How visible the bill_stats tool is to the model.
     *
     * Two rows, laid out like the surfaces rows below it: label and hint on
     * the left, one control on the right. The lookback window is a number
     * input — 0 means "never provide", N scans the recent N user messages. The
     * pattern is a text input with live validation: a source that does not
     * compile is refused and never written, so the stored preference can only
     * ever hold a valid pattern.
     */
    function ToolsSection(props) {
      var t = props.t
      var pair = useLazyTools()
      var lazy = pair[0]
      var write = pair[1]
      var lookbackDraftState = React.useState(String(lazy.lookback))
      var lookbackDraft = lookbackDraftState[0]
      var setLookbackDraft = lookbackDraftState[1]
      var lookbackFocusState = React.useState(false)
      var lookbackFocused = lookbackFocusState[0]
      var setLookbackFocused = lookbackFocusState[1]
      // The draft is the user's own text while the field is focused, so the
      // stored value is NOT adopted mid-edit: an earlier `prefs-set` reply can
      // otherwise land after a newer optimistic write and snap the draft back
      // while the user is still typing. Outside focus the stored value is
      // authoritative (the first `prefs` reply, a second window, a write from
      // elsewhere).
      React.useEffect(function () {
        if (!lookbackFocused) setLookbackDraft(String(lazy.lookback))
      }, [lazy.lookback, lookbackFocused])
      var patternDraftState = React.useState(lazy.pattern)
      var patternDraft = patternDraftState[0]
      var setPatternDraft = patternDraftState[1]
      var patternFocusState = React.useState(false)
      var patternFocused = patternFocusState[0]
      var setPatternFocused = patternFocusState[1]
      // The pattern field is a draft over a stored value, not the stored value
      // itself: while the source does not compile the preference keeps its last
      // valid value, so the two diverge by design. The stored value is adopted
      // when it changes underneath us (the first `prefs` reply, a second
      // window, a write from elsewhere) — but never while the field is
      // focused, where the draft is the user's own text and an earlier reply
      // could otherwise snap it back mid-edit.
      React.useEffect(function () {
        if (patternFocused) return
        setPatternDraft(function (current) { return current === lazy.pattern ? current : lazy.pattern })
      }, [lazy.pattern, patternFocused])
      var invalid = false
      if (patternDraft) {
        try { new RegExp(patternDraft, 'i') } catch (e) { invalid = true }
      }
      var lookbackInvalid = false
      var lookbackNum = Number(lookbackDraft)
      if (lookbackDraft !== '') {
        if (!Number.isInteger(lookbackNum) || lookbackNum < 0 || lookbackNum > 50) lookbackInvalid = true
      }
      return el('div', { style: section },
        el('div', { style: panelT }, t('tools.title')),
        el('div', { style: panelSub }, t('tools.desc')),
        el('div', { style: surfaceRow },
          el('div', { style: { minWidth: 0 } },
            el('div', { style: surfaceName }, t('tools.lazy')),
            el('div', { style: surfaceHint }, t('tools.lazy.desc'))),
          el('input', {
            className: 'dshbill-field dshbill-input',
            type: 'number',
            min: 0,
            max: 50,
            step: 1,
            ...
            onFocus: function () { setLookbackFocused(true) },
            onBlur: function () { setLookbackFocused(false) },
            onChange: function (e) {
              var next = e.target.value
              setLookbackDraft(next)
              // Empty means 0 (never provide). Only a non-negative integer
              // inside the window is written; anything else is a draft-only
              // rejection the user can correct.
              var n = next === '' ? 0 : Number(next)
              if (Number.isInteger(n) && n >= 0 && n <= 50) write({ lookback: n })
            },
            style: surfaceControl,
          })),
        lookbackInvalid
          ? el('div', { style: { ...surfaceHint, color: T.error } }, String(t('tools.lazy') + ': 0–50'))
          : null,
        el('div', { style: { ...surfaceRow, display: 'block' } },
          el('div', { style: surfaceName }, t('tools.pattern')),
          el('div', { style: surfaceHint }, t('tools.pattern.desc')),
          el('input', {
            className: 'dshbill-field dshbill-input',
            type: 'text',
            value: patternDraft,
            placeholder: '费用|cost',
            onFocus: function () { setPatternFocused(true) },
            onBlur: function () { setPatternFocused(false) },
            onChange: function (e) {
              var next = e.target.value
              setPatternDraft(next)
              // Refuse to persist a source that does not compile: the field
              // shows the error and the stored preference keeps its last
              // valid value until the draft compiles again.
              if (!next) { write({ pattern: '' }); return }
              try { new RegExp(next, 'i'); write({ pattern: next }) } catch (err) { /* keep the stored value */ }
            },
            // The pattern is prose, not a flag: it takes the whole row, so a
            // long alternation is visible without horizontal scrolling.
            style: { ...surfaceControl, display: 'block', width: '100%', marginLeft: 0, marginTop: 8, boxSizing: 'border-box' },
          })),
        invalid
          ? el('div', { style: { ...surfaceHint, color: T.error } }, t('tools.pattern.invalid'))
          : null)
    }
