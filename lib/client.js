...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var CSS = `
      ...
      .dshbill-field:focus-visible,.dshbill-pill:focus-visible,.dshbill-panel-action:focus-visible,
      ...
      @media (prefers-reduced-motion:reduce){.dshbill-btn,.dshbill-seg>button,
        .dshbill-field,.dshbill-pill,.dshbill-switch,.dshbill-switch>span,.dshbill-spend,.dshbill-table tbody tr,
    ...
    // (pushed as each turn closes, no timer), and from one `session-cost` fold
    // otherwise.
    ...
    function CostLine(props) {
      ...
      var state = useCostApi(function () {
        return { action: 'session-cost', sessionId: sessionId }
      ...
      if (d && d.calls > 0) {
        ...
        ;(d.byModel || []).slice(0, 3).forEach(function (row) {
          if (row.usd > 0) rows.push([row.displayName || row.model, fmt(row.usd)])
        })
        if (d.peakUsd > 0) {
          rows.push([t('dock.peak'), fmt(d.peakUsd)])
          rows.push([t('dock.offPeak'), fmt(d.offPeakUsd)])
        }
      }
    ...
    function SidebarSpend(props) {
      ...
      return el('button', {
        ...
        onClick: function () {
          if (!openBillPanel() && !openBillView(translate('view.tab'))) openSettingsSection(translate('section.title'))
        },
    ...
    function SessionHoverCost(props) {
      ...
      if (d.calls > 0) {
        ...
        items.push(el('span', { key: 'c' }, d.calls + t('hover.calls')))
      }
    ...
    function currencyItems(t, fx, value) {
      ...
      Object.keys(fx || {}).sort().forEach(function (code) {
        if (!seen[code]) other.push({ id: code, label: currencyLabel(code) })
      })
    ...
    function BudgetRows(props) {
      ...
      var status = t('budget.desc')
      var track = null
      if (budget.amount > 0) {
        var spent = props.spentUsd === undefined ? null : convert(props.spentUsd, budget.currency, fx)
        if (spent === null) {
          status = t('state.loading')
        } else {
          var pct = spent / budget.amount * 100
          var level = pct >= 100 ? 'over' : pct >= 80 ? 'warn' : 'ok'
          var color = level === 'over' ? T.error : level === 'warn' ? T.warn : T.business
          var remaining = budget.amount - spent
          status = el('span', { style: { color: level === 'ok' ? undefined : color } },
            t('budget.period.' + budget.period) + ' · ' + t('budget.used') + sym + spent.toFixed(2)
              + t('budget.of') + sym + budget.amount.toFixed(2) + ' · '
              + (remaining >= 0
                ? t('budget.left') + sym + remaining.toFixed(2)
                : t('budget.over') + sym + (-remaining).toFixed(2)))
          track = el('div', { className: 'dshbill-track' },
            el('div', {
              style: {
                width: Math.min(100, pct) + '%', height: '100%',
                background: color, transition: 'width .2s ' + T.ease,
              },
            }))
        }
      }
    ...
    function BudgetStatus(props) {
      ...
      var sym = CURRENCY_SYMBOL[budget.currency] || (budget.currency + ' ')
      var spent = convert(props.spentUsd, budget.currency, props.fx)
      ...
      return el('div', { style: section },
        el('div', { style: { display: 'flex', alignItems: 'baseline', gap: 8, minWidth: 0 } },
          ...
          el('span', {
            style: { marginLeft: 'auto', color: level === 'over' ? T.error : T.label, ...N.xxsStrong },
          }, t('budget.used') + sym + (spent === null ? '—' : spent.toFixed(2))
             + t('budget.of') + sym + budget.amount.toFixed(2))),
        el('div', { className: 'dshbill-track', style: { height: 6, marginTop: 8 } },
          el('div', {
            style: {
    ...
    function Dashboard(props) {
      ...
      var state = useCostApi(function () {
        return sessionScope
          ? { action: 'dashboard', rangeDays: rangeDays, sessionId: sessionScope }
          : { action: 'dashboard', rangeDays: rangeDays }
      ...
      if (d && !d.error) {
        ...
        // The account balance is not a fact about one conversation.
        if (!sessionScope && bal && bal.ok) {
    ...
    /** Open the global panel through the layout service, when it offers that. */
    var layoutService = null
    function openBillPanel() {
      if (layoutService && typeof layoutService.selectPanel === 'function') {
        try { layoutService.selectPanel('bill'); return true } catch (e) { /* fall through */ }
      }
      return false
    }
    ...
    /**
     * The settings page, as a flat list of the shipped General page's rows:
     * no page title (the dialog's nav already names it) and no section cards —
     * budget, display currency, then one switch per surface.
     */
    ...
    exports.apply = function (ctx) {
      ...
      if (layout) layoutService = layout
