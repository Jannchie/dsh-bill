...
// ── Router ids → catalogue keys ─────────────────────────────────────────────
//
// DSH stores the model string the ROUTER was addressed by, not the name the
// vendor publishes. A gateway prefixes it with its provider (`antigravity/…`)
// and appends the routing tier it actually ran (`-thinking`, `-high`,
// `-xhigh`, `-max`, `-medium`, `-low`, `-none`); Cursor additionally spells
// Claude family-last (`claude-4.6-opus` for the vendor's `claude-opus-4.6`).
// The catalogue keys on the vendor's own name, so every such call landed as
// unpriced — the model is listed and priced, we just never asked for it.
//
// This maps a stored id to the catalogue key it MEANT. Nothing is estimated:
// every candidate is looked up for real, and an id with no entry anywhere
// still comes back unpriced rather than at a guessed rate.
//
// Two rules keep it honest:
//
//   - Literal first, always. `claude-sonnet-4-6-thinking` is itself a
//     catalogue key, so a rewrite must never pre-empt an exact hit.
//   - Only ROUTING tokens are peeled. `-fast` is a real rate multiplier
//     (llm-pricing prices `claude-opus-4-7-fast` at 6x its base), and
//     `flash`/`pro`/`lite`/`mini`/`codex` are name parts — peeling those
//     would resolve to the wrong model's price, which is worse than `?`.
//     A `-fast` suffix is therefore carried through the peel, not dropped.
const ROUTING_SUFFIXES = [
  'thinking', 'xhigh', 'high', 'medium', 'low', 'minimal', 'none', 'max',
]

/** `claude-4.6-opus` → `claude-opus-4.6` (Cursor's family-last spelling). */
function familyFirst(id) {
  const m = id.match(/(^|[/-])claude-(\d+(?:\.\d+)?)-(opus|sonnet|haiku)(?=-|$)/)
  if (!m) return null
  return id.slice(0, m.index) + m[1] + `claude-${m[3]}-${m[2]}` + id.slice(m.index + m[0].length)
}

/**
 * Every catalogue key the stored id could mean, most literal first.
 *
 * Progressive: peel one routing token at a time so a shorter true name wins
 * over a more aggressive rewrite, then offer the family-first respelling of
 * each. `-fast` is remembered and re-appended, so
 * `claude-opus-4-7-high-fast` resolves to the priced `claude-opus-4-7-fast`
 * rather than to the 6x-cheaper base.
 */
function resolvedCandidates(id) {
  const out = []
  const seen = new Set([id])
  const add = (candidate) => {
    if (candidate && !seen.has(candidate)) { seen.add(candidate); out.push(candidate) }
  }
  let current = id
  let fast = false
  // 4 peels covers the deepest tier stack seen in practice; the loop exits on
  // its own once no routing token is left.
  for (let i = 0; i < 4; i++) {
    const fastMatch = current.match(/-fast$/)
    if (fastMatch) {
      fast = true
      current = current.slice(0, -'-fast'.length)
      add(fast ? `${current}-fast` : current)
      continue
    }
    const token = ROUTING_SUFFIXES.find((suffix) => current.endsWith(`-${suffix}`))
    if (!token) break
    current = current.slice(0, -token.length - 1)
    add(fast ? `${current}-fast` : current)
  }
  for (const candidate of [id, ...out]) {
    add(familyFirst(candidate))
  }
  return out
}

/** Resolved-key memo, dropped whenever the rate card underneath changes. */
let resolvedAt = -1
let resolvedCache = new Map()

/**
 * The catalogue key that prices `model`, or `model` itself when nothing does.
 *
 * One choke point for every catalogue lookup in this module, so cost, peak
 * tag and off-peak counterfactual all consult the same model identity — a
 * resolved price paired with an unresolved schedule would put a call on the
 * wrong side of a peak window without anything failing loudly.
 */
function resolveModel(model, at) {
  if (resolvedAt !== epoch) {
    resolvedAt = epoch
  ...
  let resolved = model
  if (!catalog.getPrice(model, at)) {
    for (const candidate of resolvedCandidates(String(model ?? '').toLowerCase())) {
      if (catalog.getPrice(candidate, at)) { resolved = candidate; break }
    }
  }
  resolvedCache.set(model, resolved)
  return resolved
}
...
export function ratesFor(model, at) {
  return catalog.getPrice(resolveModel(model, at), at)
}
...
export function peakStateFor(model, timeMs) {
  const schedule = catalog.getSchedule(resolveModel(model, timeMs))
...
export function priceRecord(rec) {
  const at = rec.time
  // Resolved once and reused for the price, the estimate and the peak tag:
  // `estimate` re-runs its own catalogue lookup, so handing it the raw router
  // id would price nothing, and `peakStateFor` would tag against a schedule
  // that does not exist.
  const model = resolveModel(rec.model, at)
  const price = catalog.getPrice(model, at)
  if (!price) {
    // `displayName: null`, never undefined: this object feeds the byModel fold
    // rows the bill_stats tool returns, and the host's lossless-JSON validator
    // rejects the whole tree over one own `undefined` property. Every consumer
    // already renders a falsy displayName as the model id.
    return { usd: null, priced: false, displayName: null, base: null, peak: null }
  }
