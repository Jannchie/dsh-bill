...
export default {
   ...
   * than trusted.
   */
  const maxRecords = Number(cfg.maxRecords) > 0 ? Number(cfg.maxRecords) : DEFAULT_MAX_RECORDS
  ...
  function persistRollup() {
    ...
    persistQueue = persistQueue
      .then(() => writeAtomic(rollupFile(), JSON.stringify(rollup)))
      .catch(() => { /* best effort */ })
  }
   ...
   *
   * `writeFile` truncates before it writes, so a process killed mid-write — or
   * a reader arriving mid-write — sees a half-written file. Rename is atomic
   * on the same filesystem, so a reader sees either the old file or the new
   * one. The temp name carries the pid: two dsh processes sharing one $DSH_HOME
   * must not race on the same scratch path.
   */
  ...
  function persist() {
    ...
    persistQueue = persistQueue.then(async () => {
      ...
      if (!rewrite) {
        ...
        try {
          mkdirSync(join(file, '..'), { recursive: true })
          await fsPromises.appendFile(file, text, 'utf8')
        } catch (error) {
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (action === 'dashboard') {
          ...
          // 8s from up to three mounted views at two different ranges. Keyed by
          // range so those views do not evict each other, and stamped with the
          // ring version plus a one-minute bucket — a bounded range and the
          // "today" totals both move with the clock, so a machine sitting idle
          // across midnight must not keep serving yesterday's answer.
          const stamp = `${ringVersion}:${Math.floor(Date.now() / 60000)}`
          const cached = dashboardCache.get(rangeDays)
          if (cached && cached.stamp === stamp) { sendJson({ ...base, ...cached.payload }); return }
