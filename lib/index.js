...
import { createLazyMatcher, recentText, DEFAULT_LAZY_PATTERN } from './lazy.js'
/** Default in-memory ring cap; `maxRecords` in the plugin config overrides it. */
const DEFAULT_MAX_RECORDS = 20000

/**
 * Default lookback window for new installs. A fresh install ships with the
 * cost tool provided on demand over the recent 6 user messages; only an
 * explicit 0 turns it off entirely.
 */
const DEFAULT_LAZY_LOOKBACK = 6
...
const DEFAULT_PREFS = {
  ...
  showView: true,
  showSidebar: true,
  // How many trailing USER messages the lazy-tool scan reads when deciding
  // whether to reveal `bill_stats`. 0 means "never provide" (the tool is
  // hidden from every request); a positive N scans the recent N user messages
  // and provides the tool only when the pattern matches. It is a number, not a
  // boolean, because the width of the window is a real knob: too small and a
  // follow-up like "and last week?" loses the tool, too large and the tool is
  // always visible (the very overhead lazyTools exists to avoid).
  lazyToolsLookback: 6,
  // Regular-expression source the recent user text is matched against when
  // `lazyToolsLookback` is positive. Defaults to the built-in cost pattern. An
  // empty string is a deliberate "always provide" chosen in the UI; only a
  // string survives the round trip (validity is enforced at write time).
  lazyToolsPattern: DEFAULT_LAZY_PATTERN,
}

/**
 * Coerce a stored document to the declared shape, field by field.
 *
 * A hand-edited or half-written file must not be able to crash a read or
 * smuggle a non-boolean into a surface flag, so every field is rebuilt from
 * the input rather than trusted. Out-of-range or wrong-typed values fall back
 * to the default; for `lazyToolsPattern` that means the built-in cost pattern
 * (only a string survives, and validity is enforced at write time).
 */
function normalizePrefs(raw) {
  ...
  return {
    ...
    // 0 = never provide; positive N = scan the recent N user messages. A new
    // install (no field at all) defaults to the shipped window. An explicit
    // invalid value falls back to the default window rather than to 0.
    lazyToolsLookback: input.lazyToolsLookback !== undefined
      ? (Number.isInteger(input.lazyToolsLookback) && input.lazyToolsLookback >= 0 ? input.lazyToolsLookback : DEFAULT_LAZY_LOOKBACK)
      : DEFAULT_LAZY_LOOKBACK,
    // An empty string is meaningful ("always provide", chosen in the UI) when
    // the lookback is positive, so absent is distinguished from empty: absent
    // restores the built-in pattern, empty stays empty.
    lazyToolsPattern: typeof input.lazyToolsPattern === 'string'
      ? input.lazyToolsPattern
      : DEFAULT_LAZY_PATTERN,
  }
}
...
export const Config = {
  '~standard': {
    ...
    validate(input) {
      ...
      // `lazyToolsLookback` / `lazyToolsPattern` are deliberately NOT config
      // fields. They are user preferences (the settings dialog's Tools
      // section): they have a UI, they are per-user, and a second source in
      // the profile patch would only make "why is this not what the UI shows"
      // a question. Deployment-shaped knobs (the ring cap, the pricing table)
      // stay here; anything the settings page owns lives in prefs. See
      // DEFAULT_PREFS.
...
export default {
  ...
  // ── lazy bill_stats: strip the schema from unrelated requests ────────────
  //
  // `bill_stats` is registered unconditionally; its ~2.2k-token schema is then
  // part of every model request. When the user turns the Tools toggle on,
  // this listener trims
  // it back to just the requests that look like a cost question, judged on the
  // same derived message history the model is about to receive — so the tool is
  // REVEALED exactly when the user actually asks about spend, and hidden
  // otherwise. All other tools are untouched; the registry itself is never
  // mutated (only the per-request system-prompt assembly), so execution
  // semantics, the dashboard, and the RPC channel are unaffected.
  //
  // `system-prompt/assemble` is a scoped waterfall: dsh-agent-loop fires it per
  // request with `context.agent` = the loop's AgentHandle (whose `.session` is
  // the session the tool would read). The global (scope-less) listener below is
  // reached through Cordis' scope filter (`hook.global || !filter || ...`), so
  // this one plugin instance can serve every session at once.
  // The switch is a USER PREFERENCE, not a config field: it lives in the
  // settings dialog's Tools section and persists through `prefs` (the plugin's
  // own store — see the block comment by DEFAULT_PREFS). Read per request, so
  // a settings change in one window is live in every window without a restart.
  // `lazyToolsLookback` doubles as the boolean: 0 means never provide the
  // tool, a positive N means scan the recent N user messages.
  const lazyToolsLookbackOf = () => prefs.lazyToolsLookback || 0
  {
    // Per-source matcher with its own compile cache: a settings edit is live
    // for the very next request, and a stable source costs one .test() per
    // request.
    const matchLazy = createLazyMatcher()

    ctx.on('system-prompt/assemble', (assembly, context, next) => {
      // Keep the outermost waterfall promise: run the chain first (so the final
      // assembled tools — including contributions from later listeners — are what
      // we trim), then filter. Returning a fresh array keeps `assembly` itself
      // untouched for any later listener that may hold it.
      return next().then((assembled) => {
        try {
          // Re-read per request: a settings change in the dialog is live for
          // the very next call, without a restart.
          const lookback = lazyToolsLookbackOf()
          if (lookback <= 0) {
            // 0 = never provide the tool: strip it from every request. This
            // short-circuit also skips the message-history derivation below —
            // paying nothing per request is the whole point of the setting.
            assembled.tools = assembled.tools.filter((tool) => tool?.name !== 'bill_stats')
            return assembled
          }
          const session = (context?.agent && context.agent.session) || null
          const messages = session && typeof session.deriveMessages === 'function'
            ? session.deriveMessages()
            : []
          // Only the user's own messages inform the judgement; the assistant's
          // prior answers (which may quote costs) would otherwise keep the tool
          // latched on forever. Text-less user-role entries (tool results) are
          // passed through too: `recentText` skips them without consuming a
          // lookback slot.
          const userOnly = messages.filter((message) => message?.role === 'user')
          const text = recentText(userOnly, lookback)
          // The pattern is re-read per request and compiled once per source:
          // a settings-dialog edit is live for the very next call, and the
          // cache keeps the per-request scan at one RegExp construction.
          if (!matchLazy(text, prefs.lazyToolsPattern || '')) {
            assembled.tools = assembled.tools.filter((tool) => tool?.name !== 'bill_stats')
          }
        } catch (error) {
          // A judgement failure must never break a prompt; worst case the tool
          // stays visible this request.
          console.warn('dsh-bill: lazyTools judgement failed —', error?.message ?? error)
        }
        return assembled
      })
    })
  ...
  async function handleAction(body) {
    ...
    if (action === 'prefs-set') {
      ...
      // A pattern that cannot be a regex is refused, not silently degraded:
      // the settings UI blocks the save, and a hand-rolled API call gets the
      // reason back. A non-string value (e.g. a number) would otherwise pass
      // the compile guard and silently fall back to the built-in pattern
      // inside normalization, so the type is checked too.
      if (patch.lazyToolsPattern !== undefined) {
        if (typeof patch.lazyToolsPattern !== 'string') {
          return { ...base, ok: false, error: 'invalid-pattern', message: 'lazyToolsPattern must be a string' }
        }
        if (patch.lazyToolsPattern) {
          try {
            new RegExp(patch.lazyToolsPattern, 'i')
          } catch (error) {
            return { ...base, ok: false, error: 'invalid-pattern', message: error?.message ?? String(error) }
          }
        }
      }
      return { ...base, ok: true, prefs: await writePrefs(patch) }
    }
