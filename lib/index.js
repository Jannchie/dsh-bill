...
/**
 * Maximum in-memory records (ring cap).
 *
 * Overridable so the eviction path — which folds records into the rollup and
 * is otherwise unreachable until a few months of use — can be exercised in a
 * test rather than trusted.
 */
const MAX_RECORDS = Number(process.env.DSH_BILL_MAX_RECORDS) > 0
  ? Number(process.env.DSH_BILL_MAX_RECORDS)
  : 20000
...
export default {
  ...
  /** Memoised dashboard payload, keyed by ring identity + range. */
  const dashboardCache = new Map()

  /** Serialized persistence queue (fs writes are ordered and best-effort). */
  let persistQueue = Promise.resolve()
  /** Lines already on disk, so ordinary appends never rewrite the file. */
  let persistedCount = 0
  /** Set when the ring changed shape (eviction, sort) and the file must be rewritten. */
  ...
  function rollupFile() {
    return join(recordsFile(), '..', 'rollup.json')
  }
  ...
  function foldIntoRollup(rec) {
    priceRecord(rec)
    rollup.calls++
    if (rec.usd === null) rollup.priced = false
    else rollup.usd += rec.usd
    rollup.uncachedInputTokens += rec.inputTokens ?? 0
    rollup.cacheReadTokens += rec.cacheReadTokens ?? 0
    rollup.cacheWriteTokens += rec.cacheWriteTokens ?? 0
    rollup.outputTokens += rec.outputTokens ?? 0
    if (rec.time > rollup.until) rollup.until = rec.time
    if (rec.peak === 'peak') { rollup.peakUsd += rec.usd ?? 0; rollup.peakCalls++ }
    else if (rec.peak === 'offpeak') { rollup.offPeakUsd += rec.usd ?? 0; rollup.offPeakCalls++ }

    const modelKey = `${rec.provider ?? 'unknown'}/${rec.model ?? 'unknown'}`
    const model = rollup.byModel[modelKey] ?? {
    ...
    model.usd += rec.usd ?? 0
    if (rec.usd === null) model.priced = false
    model.calls++
    model.inputTokens += rec.inputTokens ?? 0
    model.outputTokens += rec.outputTokens ?? 0
    model.cacheReadTokens += rec.cacheReadTokens ?? 0
    model.cacheWriteTokens += rec.cacheWriteTokens ?? 0
    if (rec.peak === 'peak') model.peakUsd += rec.usd ?? 0
    else if (rec.peak === 'offpeak') model.offPeakUsd += rec.usd ?? 0
    rollup.byModel[modelKey] = model

    const purpose = rec.purpose ?? 'agent'
    const pRow = rollup.byPurpose[purpose] ?? { purpose, usd: 0, calls: 0 }
    pRow.usd += rec.usd ?? 0
    pRow.calls++
    rollup.byPurpose[purpose] = pRow

    const day = new Date(rec.time).toISOString().slice(0, 10)
    const dayRow = rollup.byDay[day] ?? { day, usd: 0, calls: 0, tokens: 0 }
    dayRow.usd += rec.usd ?? 0
    dayRow.calls++
    dayRow.tokens += (rec.inputTokens ?? 0) + (rec.outputTokens ?? 0)
    rollup.byDay[day] = dayRow
   ...
   * exactly the silent-shrink bug the rollup exists to prevent.
   */
  function evictOldest(count) {
    if (count <= 0) return
    for (let i = 0; i < count && i < records.length; i++) foldIntoRollup(records[i])
  ...
  function persistRollup() {
    persistQueue = persistQueue.then(async () => {
      try {
        ...
        await fsPromises.writeFile(file + '.tmp', JSON.stringify(rollup), 'utf8')
  ...
  async function loadRollup() {
    try {
      const parsed = JSON.parse(await fsPromises.readFile(rollupFile(), 'utf8'))
  ...
  async function loadPersisted() {
    try {
      ...
      // Calls captured while this read was in flight are already in `records`
      // — and, having been appended, may also be in `loaded`. Keeping only
      // the strictly older half of the file makes the merge exact without
      // needing an identity that survives a restart.
      const oldestLive = records.length > 0 ? records[0].time : Infinity
      const history = loaded.filter((rec) => rec && rec.time < oldestLive)
      ...
      if (records.length > MAX_RECORDS) evictOldest(records.length - MAX_RECORDS)
      backfillPeak()
      rebuildSessionIndex()
      // The file and memory may now disagree (history was filtered, live rows
      // appended), so the next write reconciles both with a rewrite.
      persistRewrite = true
   ...
   * supposed to be cheap to keep. A full rewrite happens only when the ring
   * changed shape — eviction, or the sort after a backfill.
   */
  function persist() {
    persistQueue = persistQueue.then(async () => {
      try {
        const file = recordsFile()
        mkdirSync(join(file, '..'), { recursive: true })
        if (!persistRewrite && persistedCount <= records.length) {
          const fresh = records.slice(persistedCount)
          if (fresh.length === 0) return
          await fsPromises.appendFile(file, fresh.map((r) => JSON.stringify(r)).join('\n') + '\n', 'utf8')
          persistedCount = records.length
          return
        }
        // Temp file + rename: `writeFile` truncates before it writes, so a
        // process killed mid-rewrite — or a reader arriving mid-rewrite —
        // sees a half-written history. Rename is atomic on the same
        // filesystem, so the file is either the old one or the new one.
  ...
  function recordCall(options, usage, startedAt, segments, outputChars) {
    ...
    if (records.length > MAX_RECORDS) {
      evictOldest(records.length - MAX_RECORDS)
      rebuildSessionIndex()
    } else if (sessionId) {
   ...
   */
  function foldRecords(list, withRollup = false, since = 0) {
    ...
    if (withRollup && rollup.calls > 0) {
      calls += rollup.calls
      totalUsd += rollup.usd
      if (!rollup.priced) priced = false
      uncachedInputTokens += rollup.uncachedInputTokens
      cacheReadTokens += rollup.cacheReadTokens
      cacheWriteTokens += rollup.cacheWriteTokens
      outputTokens += rollup.outputTokens
      tokens += rollup.uncachedInputTokens + rollup.outputTokens
      peakUsd += rollup.peakUsd
      offPeakUsd += rollup.offPeakUsd
      peakCalls += rollup.peakCalls
      offPeakCalls += rollup.offPeakCalls
      for (const [key, row] of Object.entries(rollup.byModel)) {
        const live = byModel.get(key)
        if (!live) { byModel.set(key, { ...row }); continue }
        live.usd += row.usd
        live.calls += row.calls
        live.inputTokens += row.inputTokens
        live.outputTokens += row.outputTokens
        live.cacheReadTokens += row.cacheReadTokens
        live.cacheWriteTokens += row.cacheWriteTokens
        live.peakUsd += row.peakUsd ?? 0
        live.offPeakUsd += row.offPeakUsd ?? 0
        if (!row.priced) live.priced = false
      }
      for (const [purpose, row] of Object.entries(rollup.byPurpose)) {
        const live = byPurpose.get(purpose) ?? { purpose, usd: 0, calls: 0 }
        live.usd += row.usd
        live.calls += row.calls
        byPurpose.set(purpose, live)
      }
    }
  ...
  function attribution(list, rangeDays) {
    ...
    if (rangeDays <= 0) {
      for (const [key, usd] of Object.entries(rollup.attr)) {
        const split = key.indexOf('|')
        const cat = split > 0 ? key.slice(0, split) : key
        const sub = split > 0 ? key.slice(split + 1) : '—'
        const row = byCat.get(cat) ?? { cat, usd: 0, children: new Map() }
        row.usd += usd
        row.children.set(sub, (row.children.get(sub) ?? 0) + usd)
        byCat.set(cat, row)
        attributedUsd += usd
  ...
  function timeline(list, rangeDays) {
    ...
    for (const [day, row] of Object.entries(rollup.byDay)) {
      ...
      const live = byDay.get(day)
      if (!live) { byDay.set(day, { ...row }); continue }
      live.usd += row.usd
      live.calls += row.calls
    ...
    handler: async (req, res) => {
      ...
      try {
        ...
        if (action === 'overview') {
          ...
          const total = foldRecords(records, true)
          sendJson({
            ...base,
            sessionId,
        ...
        if (action === 'dashboard') {
          ...
          // Seven passes over the ring per request, and the browser polls every
          // 8s from up to three mounted views. The answer only changes when a
          // call is recorded, so it is memoised on (ring identity, range) and
          // the repeat views are served from one computation.
          const cacheKey = `${recordSeq}:${records.length}:${rangeDays}`
          const cached = dashboardCache.get(cacheKey)
          if (cached) { sendJson({ ...base, ...cached }); return }
          ...
          const since = rangeDays > 0 ? Date.now() - rangeDays * 86400000 : 0
          const folded = foldRecords(records, rangeDays <= 0, since)
          ...
          const payload = {
            ...
            archived: rollup.calls > 0
              ? { calls: rollup.calls, usd: roundCost(rollup.usd), until: rollup.until }
              : null,
            bySession: bySessionRows(records, rangeDays),
            perDayUsd: roundCost(folded.totalUsd / Math.max(1, rangeDays)),
          }
          // One entry: a second range replaces the first rather than growing a
          // map keyed by something a caller controls.
          dashboardCache.clear()
          dashboardCache.set(cacheKey, payload)
          sendJson({ ...base, ...payload })
          return
        }
        if (action === 'balance') {
          try {
