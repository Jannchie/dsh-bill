...
export default {
  ...
  //
  // Registered inside `child.effect`, like every route above: `webServer.register`
  // owns an entry in a shared table and hands back the disposer that removes it,
  // and it THROWS on a duplicate (kind, path). Registered bare, that disposer is
  // dropped, so the route outlives the fiber that created it. The next hot reload
