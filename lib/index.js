 ...
 *   { action: 'flush' }                    → drain the persist queue, for an
 *                                            outside reader (test / backup)
 *
...
/**
 * Default total budget for the one boot-time history import pass. The pass is
 * abortable as a unit, so this bounds how long a fresh install may spend
 * pricing its pre-install history; a warm home spends nothing (every session
 * already has records).
 */
const DEFAULT_BACKFILL_TIMEOUT_MS = 60000
...
const DEFAULT_PREFS = {
  ...
  budgetPeriod: 'month',
  budgetCurrency: 'CNY',
  // Display currency for every figure the plugin prints. Deliberately separate
  // from `budgetCurrency`: a limit is a promise made in one currency
  // ("¥100 a month") and must not restate itself when the display changes.
  currency: 'CNY',
...
function normalizePrefs(raw) {
  ...
  return {
    ...
    showSessionHover: input.showSessionHover !== false,
  }
}
...
export const Config = {
  '~standard': {
    ...
    validate(input) {
      ...
      if (input === undefined || input === null) return { value: { maxRecords: DEFAULT_MAX_RECORDS, agentTool: true, backfillTimeoutMs: DEFAULT_BACKFILL_TIMEOUT_MS } }
      ...
      // Whether `bill_stats` is registered at all. Static on purpose: the tool
      // list is the front of the prompt-cache prefix, so the only cache-safe
      // way to keep its schema off requests is to never register it. A
      // per-request toggle would invalidate the whole context every time it
      // flipped (see dsh-tools' "KV Cache effect").
      const { agentTool } = input
      if (agentTool === undefined) value.agentTool = true
      else if (typeof agentTool !== 'boolean') issues.push(issue('expected a boolean', 'agentTool'))
      else value.agentTool = agentTool

      const { backfillTimeoutMs } = input
      if (backfillTimeoutMs === undefined) value.backfillTimeoutMs = DEFAULT_BACKFILL_TIMEOUT_MS
      else if (typeof backfillTimeoutMs !== 'number' || !Number.isInteger(backfillTimeoutMs) || backfillTimeoutMs <= 0) {
        issues.push(issue('expected a positive integer', 'backfillTimeoutMs'))
      } else value.backfillTimeoutMs = backfillTimeoutMs
...
export default {
  ...
  /** Total budget for the boot-time history import; `backfillTimeoutMs` config. */
  const backfillTimeoutMs = cfg.backfillTimeoutMs ?? DEFAULT_BACKFILL_TIMEOUT_MS
  ...
  let loading = true
  /**
   * How many times a drain re-chases the queue before it gives up.
   *
   * Each pass drains what was queued when that pass started, so a few is ample
   * for real work: the queue grows by one task per capture, never by a
   * generation of them. The bound is what keeps `flush` a request rather than a
   * wait — it is reachable over plain HTTP, and a caller that must get an answer
   * cannot be made to wait out a capture stream that outpaces the disk. A drain
   * that hits the bound says so instead of handing back a file still mid-write.
   */
  ...
  function foldOne(acc, rec) {
    ...
    const row = acc.byModel[key] ?? (acc.byModel[key] = {
      // `?? null` on every record-sourced field: the row is returned by the
      // bill_stats tool, and the host's lossless-JSON validator rejects the
      // whole tree over one own `undefined` property — which a record whose
      // displayName was never set (a legacy line, a skipped re-price) would
      // otherwise carry. `rec.base` can already be null; keep that shape.
      provider: rec.provider ?? 'unknown', model: rec.model ?? 'unknown', displayName: rec.displayName ?? null, base: rec.base ?? null,
  ...
  /**
   * Top up `usd` (and peak/base/displayName) on backfilled log records that
   * arrived before the pricing catalogue loaded. Safe only for source:'log'
   * rows — a live record's stored cost is what was actually billed and is never
   * touched (see backfillPeak). Idempotent: priceRecord skips anything already
   * priced. Runs once, after the catalogue lands, on the settled ring.
   */
  function repriceLogRecords() {
    let changed = false
    for (const rec of records) {
      if (rec && rec.source === 'log' && rec.usd === null) {
        priceRecord(rec)
        if (rec.usd !== null) changed = true
      }
    }
    if (changed) {
      ringVersion++
  ...
  /**
   * Wait for every queued record append and rollup write to reach the file,
   * and for the startup read to finish.
   *
   * Capture is deliberately fire-and-forget: a recorded call returns once the
   * record is in the in-memory ring, and a serialized queue brings the file
   * up to date behind it. The dashboard reads from the ring and is never
   * affected by a lagging queue; an OUTSIDE reader (a test, a backup, an
   * export, or a second instance about to read the same home) is: it would
   * otherwise see a half-written log (fewer lines than the ring holds, a dead
   * prefix mid-compaction, or appends still landing from a prior instance
   * while a new one boots). This drain closes that gap.
   *
   * persist() and persistRollup() reassign persistQueue as they chain new
   * work, so the reference captured before an await can go stale while the
   * await is in flight; the loop re-chases until a pass adds nothing. The
   * startup read suppresses writes and ends by flipping loading and calling
   * persist(), so the queue is empty while it runs; awaiting it alone would
   * not cover its tail, hence the loading poll first.
   *
   * @returns {Promise<boolean>} whether the queue actually went quiet.
   */
  async function flushPersist() {
    // Bound the loading wait: load is a one-shot read at boot, so it flips
    // within milliseconds; a stuck loading (a load that never settled) must
    // not hang a flush forever. The drain still degrades gracefully below.
    for (let i = 0; i < 1000 && loading; i++) await new Promise((r) => setTimeout(r, 10))
    for (let pass = 0; pass < FLUSH_MAX_PASSES; pass++) {
      const draining = persistQueue
      await draining
      if (persistQueue === draining) return true
    }
    return false
  }
  ...
  //
  // `agentTool: false` skips the registration entirely — the dashboard and
  // the carriers are unaffected. Someone who never asks the model about spend
  // pays the schema on every request for nothing.
  if (cfg.agentTool !== false) ctx.inject(['tools'], (child) => {
   ...
   *
   * Runs once at startup, as soon as capture is live — it does NOT wait for
   * the pricing catalogue, because a cold fetch can hang for tens of seconds
   * or fail offline, and that must not block importing history. An imported
   * record is priced with whatever the catalogue has right now (built-in
   * overrides, a warm file cache, or nothing → usd:null unpriced), and
   * repriceLogRecords tops up the still-unpriced log rows once the catalogue
   * lands. A session that already has ANY record is skipped wholesale, which
   * keeps the pass idempotent across restarts (see the header comment above
   * for the undercounting trade-off).
   *
   * Cost model: a persisted `readSession` re-lists the ENTIRE artifact tree
   * before loading one session, so importing N missing sessions that way is N
   * full listings — O(sessions²) file walks that peg a core for tens of
   * minutes on large homes — and its per-read race timed out WITHOUT aborting,
   * so timed-out loads kept running in the background while the loop moved on,
   * piling work up behind the deadline. The replacement lists the tree ONCE
   * and then borrows each session's own artifact via `observeSession` (bounded
   * concurrency, abortable, no replay snapshot), so the pass is O(sessions)
   * file work and the shared AbortSignal makes it cancellable as a unit: a
   * budget exhaustion persists what was imported and the next boot resumes
   * exactly where `bySession` left off.
   *
   * Requires a session-query with `observeSession`; older hosts skip the
   * import rather than falling back to the per-session crawl.
   */
  async function backfillFromLog() {
    ...
    if (typeof sessionQuery.observeSession !== 'function') {
      console.warn('dsh-bill: session backfill skipped — session-query has no observeSession')
      return
    }
    const controller = new AbortController()
    const timer = setTimeout(
      () => controller.abort(new Error('backfill budget exhausted')),
      backfillTimeoutMs,
    )
    try {
      let listed
      try {
        listed = await sessionQuery.listSessions(controller.signal)
      } catch {
        if (controller.signal.aborted) {
          console.warn('dsh-bill: session backfill listing stopped at the budget; nothing imported this boot')
        }
        return
      }
      const ids = []
      for (const entry of Array.isArray(listed) ? listed : []) {
        ...
        if (id && !bySession.has(id)) ids.push(id)
      }
      if (ids.length === 0) return
      let imported = 0
      // The observation lease hands back the raw (uncloned) event log, so this
      // pass never deep-copies an event array the way readSession's replay
      // snapshot does. A small worker pool keeps the per-session artifact
      // reads bounded, and every worker stops on the shared signal.
      const CONCURRENCY = 4
      let cursor = 0
      const worker = async () => {
        for (;;) {
          ...
          const id = ids[index]
          try {
            const obs = await sessionQuery.observeSession(id, { signal: controller.signal, projectionMode: 'none' })
            try {
              for (const sample of samplesFromEvents(obs?.events)) {
                const rec = priceRecord({
                  seq: recordSeq++,
                  time: sample.time,
                  sessionId: id,
            ...
            console.warn('dsh-bill: session backfill skipped a session —', error?.message ?? error)
          }
        }
      }
      // allSettled, not all: Promise.all settles on the first abort while
      // sibling workers can still be mid-session, and a record they push after
      // the sort/persist below would sit in memory unpersisted. Waiting for
      // every worker relies on observeSession honouring the signal promptly.
      const results = await Promise.allSettled(Array.from({ length: Math.min(CONCURRENCY, ids.length) }, () => worker()))
      if (controller.signal.aborted) {
        // Budget exhausted mid-pass: keep what was imported and let the next
        // boot resume where bySession left off.
        const reason = controller.signal.reason
        console.warn('dsh-bill: session backfill stopped at the budget —', reason?.message ?? reason)
      } else {
        // Workers only reject on abort today; surface anything else as a failure.
      ...
      // The imported rows land throughout the ring, so the file can no longer
      // be reconciled by appending.
      persistRewrite = true
      ringVersion++
      if (records.length > maxRecords) evictOldest(records.length - maxRecords)
      rebuildSessionIndex()
      ...
      if (controller.signal.aborted) {
        console.warn('dsh-bill: session backfill stopped at the budget —', error?.message ?? error)
      } else {
        console.warn('dsh-bill: session backfill failed —', error?.message ?? error)
      }
    } finally {
      clearTimeout(timer)
    }
  ...
  async function handleAction(body) {
    ...
    // A drain for an outside reader (a test, a backup, an export): wait for
    // every queued append and rollup write to reach the file, and for the
    // startup read to finish, so the file is coherent rather than mid-write.
    // It bypasses the pricing/fx load every other action needs: a drain never
    // answers a cost question, and forcing that load would make a flush slow
    // and side-effectful for no reason.
    if (action === 'flush') {
      // `drained: false` means captures outpaced the disk for the whole bounded
      // window, so the file is still catching up. A caller about to read it has
      // to know that rather than infer it from a count that looks short.
      return { ok: true, drained: await flushPersist() }
    }
    await Promise.all([ensurePricingLoaded(), ensureFxLoaded()])
    const fx = getFx()
    ...
    if (action === 'dashboard') {
      ...
      // With a session id the same report is built over that session's records
      // alone — the conversation's own tab. Account-wide figures (the budget
      // periods, the forecast, the per-session split, the archived rollup)
      // have no meaning inside one session and come back empty.
      ...
      // yesterday's answer. Only the account-wide report is cached: a session
      // folds its own few records, and caching one per session visited would
      // evict the expensive global entries.
      const stamp = `${ringVersion}:${Math.floor(Date.now() / 60000)}`
      const cached = sessionId ? undefined : dashboardCache.get(rangeDays)
      if (cached && cached.stamp === stamp) return { ...base, ...cached.payload }
      const scope = sessionId ? (bySession.get(sessionId) ?? []).map((i) => records[i]) : records
      ...
      const folded = foldRecords(scope, rangeDays, !sessionId && rangeDays <= 0)
      const tl = timeline(scope, rangeDays)
      const hm = heatmap(scope, rangeDays)
      const payload = {
        rangeDays,
        ...folded,
        ...
        heatmap: hm,
        sessionId,
        attribution: attribution(scope, rangeDays),
        forecast: sessionId ? null : forecast(records, rangeDays),
        periods: sessionId ? null : periodTotals(records),
        // What the range asked for versus what the ring can still answer.
        archived: !sessionId && rollup.calls > 0 ? { calls: rollup.calls } : null,
        bySession: sessionId ? [] : bySessionRows(records, rangeDays),
  ...
  //
  // On DSH 0.1.5 (rc.1 and rc.2) `rpc.handle` throws for every caller: it
  // mounts the route through `owner.webServer`, where `owner` is the service's
  // shadow context and resolves services from the Connection plugin's OWN
  // fiber, which injects only `credentials`. Left alone, that throw kills this
  // child fiber without a word, the channel is never mounted, and the browser
  // half — which was handed a perfectly real `connection.rpc` — reports HTTP
  // 405 on every call (issue #1). So the failure is caught and named here, and
  // the HTTP carrier below stays the one that actually serves the page.
  ctx.inject(['connection'], (child) => {
    child.effect(() => {
      let dispose
      try {
        dispose = child.connection.rpc.handle(
          '/dsh-bill',
        ...
        console.warn('dsh-bill: rpc channel /dsh-bill not mounted, serving over /dsh-bill/api only —', error?.message ?? error)
        return () => {}
      }
  ...
  //
  // Registered inside `child.effect`, like the tool and the RPC channel above:
  // `webServer.register` owns an entry in a table shared by the whole host and
  // hands back the disposer that removes it, and it THROWS on a duplicate
  // (kind, path). Registered bare, that disposer is dropped, so the route
  // outlives the fiber that created it. The next hot reload
  // then rebuilds this plugin, the rebuild's `register` hits the leaked entry,
  // and the throw takes down the new generation — cordis rolls the fiber back to
  // the previous one, whose module instance is now the only one serving. The
  // dashboard keeps answering, so nothing looks broken, but every number comes
  // from the superseded module: on-disk fixes appear to have no effect until the
  // whole host is restarted.
  ...
  hostkitReady
    ...
    // Backfill runs as soon as capture is live and does NOT wait for the
    // pricing catalogue: a cold fetch (models.dev, ~4 MB, possibly offline or
    // firewalled) must not block importing history. Each imported record is
    // priced with whatever the catalogue has right now — built-in overrides, a
    // warm file cache, or nothing (usd:null, shown as unpriced) — and
    // repriceLogRecords tops up the still-unpriced log rows once it lands.
    .then(() => backfillFromLog())
    // Pricing and FX load in the background afterwards; FX is display-only and
    // never gates a record. repriceLogRecords runs after the catalogue is ready
    // (and only after backfill has settled, so it sees every imported log row).
    .then(() => Promise.all([
      ensurePricingLoaded().then(repriceLogRecords).catch(() => {}),
      ensureFxLoaded().catch(() => {}),
    ]))
    .catch(() => {})
  },
}
