...
export default {
  ...
  async function backfillFromLog() {
    ...
    try {
      ...
      if (firstErr) {
        if (!controller.signal.aborted) throw firstErr.reason
        // Budget exhausted mid-pass: keep what was imported and let the next
        // boot resume where bySession left off.
        console.warn('dsh-bill: session backfill stopped at the budget —', firstErr.reason?.message ?? firstErr.reason)
      }
