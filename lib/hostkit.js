...
/**
 * Import a package that exists only inside a host profile.
 *
 * The same constraint `hostkitReady` handles, exposed for the one-off
 * borrowings elsewhere: `@deepseek-ai/*` resolves from the profile's
 * node_modules, so a bare static import takes the whole module down under a
 * `link:`ed checkout. Resolves to `null` instead of throwing.
 */
